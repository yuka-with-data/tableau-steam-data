...
# Combine main_rgx with steam_main_data
new_steam_df <- cbind(steam_main_data, main_rgx)

## Run sentiment analysis and restructure data
# Split review text data into sentences
sent_txt <- get_sentences(new_steam_df$new_review)
# assign sentiment values
sent_txt <- sentiment_by(sent_txt)
# summary stat of ave_sentiment
summary(sent_txt$ave_sentiment) 
# check correlation between word count and sentiment value
plot(sent_txt$word_count, sent_txt$ave_sentiment)
# restructure the sent_txt data by extracting ave_sentiment and word_count columns
sent_txt <- sent_txt %>% select(ave_sentiment, word_count)
# add sentiment values and word count to the main data set
new_steam_df <- cbind(new_steam_df, sent_txt)
# remove review columns
new_steam_df <- new_steam_df %>% select(-c(review)) 
...
# write.csv(new_steam_df, ""C:\\Users\\your_file_path\\\\new_steam_df.csv", row.name = FALSE")
